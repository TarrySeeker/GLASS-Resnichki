...
export default async function InfoPage({
  ...
  /* Кадр остался на одной странице из пяти — у политики, где он показывает
     то, о чём страница и говорит: коробку не вскрывают. На остальных текст
     отвечает на вопрос, и фотография рядом с ответом ничего не прибавляла.

     Заголовок и первая строка выходят из колонки на всю ширину там, где ниже
     идёт что-то во всю ширину: у доставки таблица, у бренда — три столбца
     рассказа. */
  const art = slug === 'privacy'
  const wide = story || slug === 'delivery'
        ...
        <article className="sec">
          <div className="wrap">
            ...
            <div className="mt-8 grid gap-10 lg:grid-cols-12 lg:gap-[var(--col-gap)]">
              {wide ? (
                <>
                  ...
                  {/* Строка бренда о себе — набором заголовка, между именем
                      ...
                      себе, а не о товаре. */}
                  {'claim' in page ? <p className="t-h3 lg:col-span-12">{page.claim}</p> : null}
              ...
              <div
                /* Мера в 36em держится там, где текст отвечает на вопрос:
                   без кадра рядом полоса разъезжается на всю сетку, и строка
                   перестаёт читаться. Два исключения. Доставка: текст идёт
                   тремя столбцами, и каждый держит меру сам. Бренд: строки
                   набраны во всю ширину колонки по решению клиента — колонка
                   начинается на пятой и доходит до края сетки. */
                className={`[container-type:inline-size] lg:self-start ${
                  wide ? 'lg:col-span-12' : 'max-w-[36em] lg:col-span-6'
                }`}
              >
                {wide ? null : (
                  <>
                ...
                {'body' in page ? (
                  /* Три абзаца встают тремя столбцами. У доставки это три
                     независимых уточнения к таблице, у бренда — подход,
                     свойство и состав линеек: ни то ни другое не читают
                     подряд, и столбиком они выглядели бы продолжением текста.

                     Над каждым столбцом линия — та же, что отделяет строки
                     таблицы и позиции футера. Она и делает из трёх абзацев три
                     вещи: без неё столбцы читаются одной колонкой, разорванной
                     по случайным местам. */
                  <div
                    className={
                      wide
                        ? 'mt-14 grid gap-x-[var(--col-gap)] gap-y-10 lg:grid-cols-12'
                  ...
                  >
                    {page.body.map((par) => (
                      <p
                        key={par}
                        className={`t-lead t-muted ${
                          wide
                            ? 't-lead-wide border-t border-[var(--color-rule)] pt-6 lg:col-span-4'
                            : ''
                        }`}
                      >
                        {par}
                ...
                <Link href={CTA[slug].href(lang)} className={`btn btn-wide ${wide ? 'mt-14' : 'mt-10'}`}>
                  {CTA[slug].label(t)}
                </Link>
              </div>
              ...
              {/* На странице бренда кадр живёт иначе: он лежачий, во всю
                  ...
                  пришли. */}
              {/* У бренда кадр стоит у левого поля страницы — на той же
                  линии, что крошки, заголовки и всё остальное на сайте. За
