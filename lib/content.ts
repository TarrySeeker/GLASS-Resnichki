...
const ru: Dict = {
  ...
  info: {
    ...
    delivery: {
      ...
      body: [
        'Способ выбирает не покупатель, а карта: СДЭК работает по России и СНГ, за их пределами посылку принимает только почта. Поэтому в корзине для одних стран есть выбор между курьером и пунктом выдачи, а для других его нет.',
        'СНГ здесь — Беларусь, Казахстан, Азербайджан, Армения, Кыргызстан, Узбекистан и Таджикистан.',
        'Пункт выдачи дешевле курьера и удобнее тем, кого не застать дома.',
        'Стоимость и срок считает перевозчик по адресу — до его ввода их не знает никто. Поэтому они появляются на оформлении, а не в карточке товара.',
  ...
  info: {
    ...
    delivery: {
      ...
      body: [
        ...
        'A pickup point costs less than a courier and suits anyone hard to catch at home.',
        'The carrier works out the cost and the time from the address — until it is in, nobody knows them. That is why both appear at checkout rather than on the product page.',
  ...
  info: {
    ...
    delivery: {
      ...
      body: [
        ...
        'ونقطة الاستلام أرخص من المندوب وأنسب لمن يصعب لقاؤه في البيت.',
        'الناقل يحسب التكلفة والمدة من العنوان — وقبل إدخاله لا يعرفهما أحد. لذلك تظهران عند إتمام الطلب لا في صفحة المنتج.',
